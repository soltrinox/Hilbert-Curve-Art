...
class HilbertApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Hilbert Curve Mapping Gallery")
        ...
        ttk.Scale(controls_frame, from_=1, to=6, orient=tk.HORIZONTAL, variable=self.order, command=self.on_param_change).pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.order_label = ttk.Label(controls_frame, text=f'{self.order.get()}')
        self.order_label.pack(side=tk.LEFT, padx=5)

        ttk.Button(controls_frame, text="Save All SVGs", command=self.save_all_svgs).pack(side=tk.RIGHT, padx=10)
    ...
    def update_plot(self):
        ...
        # Plot original
        ax_orig = self.axs.flat[0]
        ax_orig.plot([p[0] for p in normalized_points], [p[1] for p in normalized_points], 'b-')
        ax_orig.set_title(f'Original (Order {order})')
        ax_orig.axis('equal'); ax_orig.grid(True)

        # Plot mappings
        for i, (name, func) in enumerate(MAPPINGS.items()):
            ax = self.axs.flat[i + 1]
            points = func(normalized_points)
            ax.plot([p[0] for p in points], [p[1] for p in points], 'b-')
            ax.set_title(f'{name} (Order {order})')
            ax.axis('equal'); ax.grid(True)
        
        # Hide unused subplot
        self.axs.flat[8].set_visible(False)
    ...
    def save_all_svgs(self):
        ...
        for name, func in MAPPINGS.items():
            points = func(normalized_points)
            filename = f'{name.lower().replace(" ", "_")}_order_{order}.svg'
            self.save_single_svg(points, output_folder, filename)

        feedback_label = ttk.Label(self.root, text=f"Saved 8 SVGs to {output_folder}", foreground="green")
